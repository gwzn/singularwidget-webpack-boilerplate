...
This repository is an **unofficial** boilerplate project for building Singular widgets using Node.js and Webpack. For more information see: https://developer.singular.live/widget-sdk/widget-sdk/

This project is in very early development currently and should be considered to be in 'alpha' stage of development. 
...
- **ALSO IMPORTANT:** Since the Singular Widget requires the files to be published to reside in a directory named "source", we are using a setup where the editable source files live in a folder called "dev" and are then bundled into the "source" folder.
...
The remainder of this README is the **Widget SDK Documentation** that is provided when executing the ```singular createwidget <widgetname>``` with the Node package ```singular-cli``` (version 0.4.0) installed:
